...
const filesWithExtension = (directory, extension) => readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
  const path = resolve(directory, entry.name)
  return entry.isDirectory() ? filesWithExtension(path, extension) : entry.name.endsWith(extension) ? [path] : []
})

rmSync(resolve(packageRoot, "dist"), { recursive: true, force: true })
const declarations = spawnSync(process.execPath, [tsc, "-p", "tsconfig.json"], { cwd: packageRoot, stdio: "inherit" })
if (declarations.status !== 0) process.exit(declarations.status ?? 1)

await build({ entryPoints: filesWithExtension(resolve(packageRoot, "src"), ".ts"), outbase: resolve(packageRoot, "src"), outdir: resolve(packageRoot, "dist/cjs"), format: "cjs", bundle: false, platform: "neutral", target: "es2022", sourcemap: true })
