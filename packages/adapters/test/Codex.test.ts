...
describe("Codex", () => {
  ...
  it("classifies Codex authentication wording", () => {
    ...
    expect(error?._tag).toBe("@smthrs-plugins/adapters/AuthFailed")
  })
  ...
  describe("preflight", () => {
    const failureOf = (exec: Parameters<typeof probeOf>[0]) =>
      Effect.runSync(Effect.flip(spec.preflight!(probeOf(exec), { CODEX_HOME: "/tmp/codex" })))

    it("passes on exit 0 and distinguishes not-executable, config, and spawn failures", () => {
      expect(
        Effect.runSync(
          spec.preflight!(exits(0), {})
        )
      ).toBeUndefined()
      expect(failureOf(() => ({ exitCode: 126 }))).toMatchObject({
        _tag: "@smthrs-plugins/adapters/BinaryMissing"
      })
      expect(failureOf(() => ({ exitCode: 3 }))).toMatchObject({
