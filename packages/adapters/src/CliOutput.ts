/**
 * Normalizes structured command-line model output into neutral records.
 *
...
/**
 * Normalized token accounting for one record, in the shape
 * `@smthrs/model`'s usage event carries.
 *
 * The projection from records to harness events lives with the consumer that
 * owns the event vocabulary, not here: an adapter's job ends at the neutral
 * record, and a package that pinned one version of the harness's event classes
 * would have to be re-released whenever they changed.
 *
 * @category destructors
 * @since 1.0.0
 */
