...
export interface Options {
   ...
   *
   * Placement belongs to the adapter, not to the runner: Claude Code takes it
   * as the last positional, Codex reads it from standard input behind its `-`
   * sentinel. A runner that appended it uniformly would hand Codex a second
   * positional and be refused.
   */
  readonly prompt?: string | undefined
  /**
   * The per-account configuration directory the vendor CLI reads.
   *
   * This is the seat: two subscriptions of the same vendor differ only by this
   * path. An adapter falls back to a workspace-local isolated directory when
   * the caller names none, so an unpooled run still never writes the operator's
   * own configuration.
   */
