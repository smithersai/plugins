...
const buildCommand = (options: Options, resume?: ResumeState): CommandSpec => {
  ...
  return {
    ...
    args: [
      ...
      // `codex exec [OPTIONS] [PROMPT]`: the prompt is the positional, and `-`
      // is the sentinel that reads it from standard input instead. Sending
      // both is refused as a second positional, and the stdin path additionally
      // races the CLI's own three-second wait for input.
      options.prompt ?? "-"
    ],
...
const preflight = (
  ...
  probe.exec("codex --version", { env: environment }).pipe(
    Effect.mapError(() =>
